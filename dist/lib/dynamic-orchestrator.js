...
exports.createDynamicOrchestrator = createDynamicOrchestrator;
...
class DynamicOrchestrator {
    constructor(agentRouter, config) {
        this.ai = google_genai_config_1.ai;
        this.agentRouter = agentRouter;
        this.intentRouter = new intelligent_intent_router_1.IntelligentIntentRouter(agentRouter);
     ...
     */
    async orchestrate(userInput, sessionId, userId) {
        try {
            ...
            return this.createErrorResult(sessionId, userId, error);
        }
    }
    /**
     ...
     */
    updateConversationHistory(session, userInput) {
     ...
     */
    async optimizeToolSelection(tools, session) {
        ...
        if (session.activeTools.length > 0) {
            const continuityTools = session.activeTools.filter(activeTool => tools.some(newTool => newTool.name === activeTool.name));
            // Combinar herramientas de continuidad con nuevas
            const newTools = tools.filter(tool => !session.activeTools.some(activeTool => activeTool.name === tool.name));
     ...
     */
    async updateSessionContext(session, selectedAgent, tools) {
     ...
     */
    async updateDominantTopics(session) {
        ...
        try {
            ...
            const topics = entityResult.entities
                ...
                .slice(0, 5);
            session.sessionMetadata.dominantTopics = [...new Set([
                    ...topics,
     ...
     */
    async generateRecommendations(orchestrationResult, session) {
        try {
     ...
     */
    buildRecommendationPrompt(orchestrationResult, session) {
...
Genera recomendaciones en el siguiente formato JSON:
{
  ...
  "clinicalConsiderations": ["Consideración clínica 1", "Consideración clínica 2"]
     ...
     */
    createErrorResult(sessionId, userId, error) {
 ...
 */
function createDynamicOrchestrator(agentRouter, config) {
